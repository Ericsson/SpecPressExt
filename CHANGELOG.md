...
- **SpecPress update** - Inherit latest SpecPress library version which by default embeds images in BASE64 encoding into exported HTML files instead of a separate "media" folder and which changes the image-width statement to the `{width=50%}` syntax (e.g. `![alt](image.png){width=50%}`).
